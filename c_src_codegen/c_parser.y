...
%token T_NUMBER T_VARIABLE T_LABEL T_CHAR_PRIMITIVE T_ADDRESS
...
%type <string> T_VARIABLE T_LABEL T_INT T_RETURN T_CHAR_PRIMITIVE T_ADDRESS
...
ASSIGN : CONTAINER T_ASSIGN LOGIC { $$ = new Assign($1, $3); }
	...
	| CONTAINER T_SUBASSIGN LOGIC { $$ = new SubAssign($1, $3); }
	| CONTAINER T_MULTASSIGN LOGIC { $$ = new MultAssign($1, $3); }
	| CONTAINER T_DIVASSIGN LOGIC { $$ = new DivAssign($1, $3); }
	| CONTAINER T_MODASSIGN LOGIC { $$ = new ModAssign($1, $3); }
	| CONTAINER T_ANDASSIGN LOGIC { $$ = new AndAssign($1, $3); }
	| CONTAINER T_ORASSIGN LOGIC { $$ = new OrAssign($1, $3); }
	| CONTAINER T_XORASSIGN LOGIC { $$ = new XorAssign($1, $3); }
	| CONTAINER T_SLLASSIGN LOGIC { $$ = new LeftShiftAssign($1, $3); }
	| CONTAINER T_SRLASSIGN LOGIC { $$ = new RightShiftAssign($1, $3); }
	| VALUE T_INCREMENT { $$ = new Increment($1); }
	| VALUE T_DECREMENT { $$ = new Decrement($1); }
...
CONTROL : ELSE { $$ = $1; }
	...
	| T_FOR T_LBRACKET ASSIGN T_SEMICOLON LOGIC T_SEMICOLON ASSIGN T_RBRACKET T_LCURLBRACKET STATEMENT_LIST T_RCURLBRACKET { $$ = new ForWithAssign($3, $5, $7, $10); }
	| T_FOR T_LBRACKET LOGIC T_SEMICOLON ASSIGN T_RBRACKET T_LCURLBRACKET STATEMENT_LIST T_RCURLBRACKET { $$ = new For ($3, $5, $8); }
...
VALUE_FOR_STORE: T_VARIABLE { $$ = new Variable (*$1, 1); }
	| T_MULT T_VARIABLE { $$ = new Pointer(*$2, 1); }
